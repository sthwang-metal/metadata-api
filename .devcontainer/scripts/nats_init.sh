...
echo "Creating NATS account for metadata-api"
nsc add account -n META -K ${OPERATOR_SIGNING_KEY_ID}
nsc edit account META --sk generate --js-mem-storage -1 --js-disk-storage -1 --js-streams -1 --js-consumer -1
nsc describe account META

export ACCOUNTS_SIGNING_KEY_ID=`nsc describe account META -J | jq -r '.nats.signing_keys | first'`

echo "Creating NATS user for metadata-api"
