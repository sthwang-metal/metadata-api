...
type AnnotationQuery struct {
	...
	withNamespace *AnnotationNamespaceQuery
	withMetadata  *MetadataQuery
	modifiers     []func(*sql.Selector)
	loadTotal     []func(context.Context, []*Annotation) error
...
func (aq *AnnotationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Annotation, error) {
	...
	if query := aq.withNamespace; query != nil {
		if err := aq.loadNamespace(ctx, query, nodes, nil,
			func(n *Annotation, e *AnnotationNamespace) { n.Edges.Namespace = e }); err != nil {
			return nil, err
		}
	}
	if query := aq.withMetadata; query != nil {
		if err := aq.loadMetadata(ctx, query, nodes, nil,
			func(n *Annotation, e *Metadata) { n.Edges.Metadata = e }); err != nil {
			return nil, err
		}
	}
...
func (aq *AnnotationQuery) loadNamespace(ctx context.Context, query *AnnotationNamespaceQuery, nodes []*Annotation, init func(*Annotation), assign func(*Annotation, *AnnotationNamespace)) error {
	ids := make([]gidx.PrefixedID, 0, len(nodes))
	nodeids := make(map[gidx.PrefixedID][]*Annotation)
	for i := range nodes {
		fk := nodes[i].AnnotationNamespaceID
	...
	query.Where(annotationnamespace.IDIn(ids...))
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "annotation_namespace_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (aq *AnnotationQuery) loadMetadata(ctx context.Context, query *MetadataQuery, nodes []*Annotation, init func(*Annotation), assign func(*Annotation, *Metadata)) error {
	ids := make([]gidx.PrefixedID, 0, len(nodes))
	nodeids := make(map[gidx.PrefixedID][]*Annotation)
	for i := range nodes {
...
func (aq *AnnotationQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(annotation.Table, annotation.Columns, sqlgraph.NewFieldSpec(annotation.FieldID, field.TypeString))
	_spec.From = aq.sql
	...
	if fields := aq.ctx.Fields; len(fields) > 0 {
		...
		if aq.withNamespace != nil {
			_spec.Node.AddColumnOnce(annotation.FieldAnnotationNamespaceID)
		}
		if aq.withMetadata != nil {
			_spec.Node.AddColumnOnce(annotation.FieldMetadataID)
		}
	}
